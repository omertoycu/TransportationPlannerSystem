...
def haversine(lat1, lon1, lat2, lon2):
    print(f"Mesafe hesaplanıyor: ({lat1}, {lon1}) -> ({lat2}, {lon2})")
    ...
    print(f"Hesaplanan mesafe: {distance:.2f} km")
    return distance
...
def find_nearest_stop(user_coord, stops):
    print(f"\nEn yakın durak aranıyor: {user_coord}")
    min_distance = float("inf")
    nearest = None
    
    for stop in stops:
        dist = haversine(user_coord["lat"], user_coord["lng"], stop["lat"], stop["lon"])
        print(f"Durak: {stop['name']}, Mesafe: {dist:.2f} km")
        
        if dist < min_distance:
            min_distance = dist
            nearest = stop
    
    print(f"En yakın durak bulundu: {nearest['name']}, Mesafe: {min_distance:.2f} km")
...
def calculate_fare(distance, vehicle_type, passenger_type, is_transfer=False, total_journey_distance=None):
    """Toplu taşıma ücretini hesaplar."""
    base_fare = 7.0  # Temel ücret
    ...
    # Aktarma teşvikleri
    if is_transfer:
        # Kısa mesafe aktarmaları için ekstra teşvik (negatif ücret)
        if distance <= 2.0:  # 2 km'ye kadar olan aktarmalar
            transfer_bonus = -2.0  # 2 TL bonus
        elif distance <= 5.0:  # 5 km'ye kadar olan aktarmalar
            transfer_bonus = -1.0  # 1 TL bonus
        else:
            transfer_bonus = 0
            base_fare *= 0.5  # Normal aktarma indirimi

        # Toplam yolculuk mesafesine göre ek teşvik
        if total_journey_distance and total_journey_distance > 10.0:
            transfer_bonus -= 1.0  # Uzun yolculuklarda ek 1 TL bonus
    else:
        transfer_bonus = 0
    
    # Mesafeye göre ek ücret (her 5 km için 1 TL)
    distance_fare = (distance // 5) * 1.0
    
    # Toplam ücret hesaplama
    total_fare = (base_fare + distance_fare) * discounts.get(passenger_type, 1.0) + transfer_bonus
    
    # Minimum ücret kontrolü (negatif ücret olabilir ama -5 TL'den az olamaz)
    total_fare = max(total_fare, -5.0)
...
def plan_route(start_coord, end_coord, stops, taxi_info, passenger_type="Genel", payment_info=None, taxi_threshold=3.0):
    ...
    print(f"\nRota planlanıyor:")
    print(f"Başlangıç: {start_coord}")
    print(f"Hedef: {end_coord}")
    print(f"Yolcu tipi: {passenger_type}")
    ...
    if best_route:
        print(f"\nEn uygun rota bulundu:")
        print(f"Tip: {best_route['type']}")
        print(f"Toplam maliyet: {best_route['total_cost']:.2f} TL")
        print(f"Toplam süre: {best_route['total_time']} dk")
        return best_route["steps"], best_route["polyline"], best_route["stops"]
    
    # Hiç rota bulunamazsa direkt taksi öner
    taxi_cost = taxi_info["openingFee"] + (direct_distance * taxi_info["costPerKm"])
    ...
    return [{
        ...
        "to": {"lat": end_coord["lat"], "lng": end_coord["lng"]},
        "cost": taxi_cost,
        "time": int(direct_distance * 3),
        "info": f"Taksi: {taxi_cost:.2f} TL (mesafe: {direct_distance:.2f} km)"
    }], [[start_coord["lat"], start_coord["lng"]], 
         [end_coord["lat"], end_coord["lng"]]], []